...
/// the data states
/// This struct contains the local, storage, and db as attributes
pub struct DateStates {
    /// local storage
    pub local: Arc<DateMapper>,
    /// cloud storage
    pub storage: Arc<DateMapper>,
    /// cloud DB
    pub db: Arc<DateMapper>,
}

impl Default for DateStates {
    fn default() -> Self {
        Self {
            local: Arc::new(DateMapper::default()),
            storage: Arc::new(DateMapper::default()),
            db: Arc::new(DateMapper::default()),
        }
    }
}

/// used to keep lookups
...
impl DateMapper {
    ...
    pub fn add_objects(
        ...
        let mut current_objects = self.read_objects(year, month, day)?;
        let provided_objects = objects.into_iter().collect::<HashSet<String>>();
        ...
        match lock.get_mut(&year_month_day) {
            Some(current_object) => {
                current_object.extend(current_objects);
            }
            None => {
                lock.insert(year_month_day, current_objects.clone());
            }
        };
...
mod tests {
    ...
    fn test_add_objects() {
        ...
        mapper
            ...
            .unwrap();
        mapper
            .add_objects(year, month, day, vec!["4".to_string(), "5".to_string()])
            .unwrap();

        // Act
        let new_objects = mapper.objects(year, month, day).unwrap();

        // Assert
        assert_eq!(new_objects, vec!["3", "4", "5"]);
    }
}
